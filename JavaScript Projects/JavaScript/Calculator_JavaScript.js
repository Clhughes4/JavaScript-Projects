...
const Calculator = {
    ...
    // this will holf the operator, we set it to null for now
    operator: null,
};
...
function Input_Digit(digit) {
    ...
    // we are checking to see if Wait_Second_Operand is tru and set
...
function Handle_Operator(Next_Operator) {
    ...
    // diplayed on the screen to a number and then store the result in the
    // Calculator.First_Operand if it doesn't already exist
    const Value_of_Input = parseFloat(Display_Value);
    // checks if an operator already exists and if Wait_Second_Operand is true
    // then updates the oerator and exists from the  function
